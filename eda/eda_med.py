...
def med_category_vs_los(df: pd.DataFrame, level, level_decile: pd.DataFrame, topK_freq=20, violin=True):
  ...
  topK_frequent_med = level_decile.sort_values(by=med_cnt, ascending=False) \
    .nlargest(topK_freq, med_cnt)
  ...
  ax.set_title("LoS Distribution by Medication Type (%d most frequent)" % topK_freq, fontsize=18, y=1.01)
